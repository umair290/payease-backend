...
class KYC(db.Model):
    ...
    # 🔐 Encrypted fields → use TEXT (no limit issues)
    cnic_number = db.Column(db.Text, unique=True, nullable=False, index=True)
    full_name_on_card = db.Column(db.Text, nullable=True)
    date_of_birth = db.Column(db.Text, nullable=True)  # or db.Date if not encrypted
