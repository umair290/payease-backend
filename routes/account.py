...
<p style="color:#AAB0C0;font-size:11px;margin:0;">payease.space</p>
    ...
    try:
        resend.Emails.send({
            ...
            "to":      [email],
            "subject": f"PayEase — PKR {amount:,.0f} Deposited Successfully",
            "html":    html,
        })
...
def send_transfer_email_sender(email, full_name, amount, recipient_name, to_wallet, new_balance, ref):
    """Email to sender after transfer."""
    now = datetime.utcnow().strftime('%d %b %Y, %H:%M UTC')
    html = f'''<!DOCTYPE html>
<html><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#F0F4FF;font-family:-apple-system,BlinkMacSystemFont,sans-serif;">
...
<p style="color:#AAB0C0;font-size:11px;margin:0;">payease.space</p>
    ...
    try:
        resend.Emails.send({
            "from":    f"PayEase <{SENDER_EMAIL}>",
            "to":      [email],
            "subject": f"PayEase — PKR {amount:,.0f} Sent to {recipient_name}",
            "html":    html,
        })
    except Exception as e:
...
def send_transfer_email_receiver(email, full_name, amount, sender_name, ref):
    """Email to receiver when money arrives."""
    now = datetime.utcnow().strftime('%d %b %Y, %H:%M UTC')
    html = f'''<!DOCTYPE html>
<html><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#F0F4FF;font-family:-apple-system,BlinkMacSystemFont,sans-serif;">
...
<p style="color:#AAB0C0;font-size:11px;margin:0;">payease.space</p>
    ...
    try:
        resend.Emails.send({
            "from":    f"PayEase <{SENDER_EMAIL}>",
            "to":      [email],
            "subject": f"PayEase — PKR {amount:,.0f} Received from {sender_name}",
            "html":    html,
        })
    except Exception as e:
...
def deposit():
    ...
    try:
        ...
        try:
            ...
            add_notification(
                user_id,
                title   = f"Deposit Successful",
                message = f"PKR {amount:,.0f} has been added to your wallet. New balance: PKR {wallet.balance:,.2f}",
                notif_type = 'success',
                icon    = 'deposit'
            )
        except Exception as e:
...
def send_money():
    ...
    try:
        # ── Lock both wallets in order to prevent deadlocks ──
        ...
        try:
            ...
            add_notification(
                user_id,
                title   = "Money Sent",
                message = f"PKR {amount:,.0f} sent to {receiver_user.full_name if receiver_user else to_wallet_number}. New balance: PKR {locked_sender.balance:,.2f}",
                notif_type = 'success',
                icon    = 'send'
            )
            ...
            if receiver_user:
                add_notification(
                    locked_receiver.user_id,
                    title   = "Money Received",
                    message = f"PKR {amount:,.0f} received from {user.full_name}.",
                    notif_type = 'success',
                    icon    = 'receive'
                )
        except Exception as e:
        ...
        # ── Emails ──
        try:
            send_transfer_email_sender(
                email          = user.email,
                full_name      = user.full_name,
                amount         = amount,
                recipient_name = receiver_user.full_name if receiver_user else to_wallet_number,
                to_wallet      = to_wallet_number,
                new_balance    = float(locked_sender.balance),
                ref            = ref
            )
        except Exception as e:
...
def lookup_wallet():
    ...
    return jsonify({
        ...
        "kyc_verified":  user.kyc_verified,
        "avatar_url":    user.avatar_url,   # ← included for profile pic feature
    }), 200
